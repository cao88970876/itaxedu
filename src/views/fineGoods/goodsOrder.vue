    ...
    <div class="multipleBuy_page page-content">
        ...
        <div class="address">
            <div class="title clear">
                <span class="left">请选择收货地址</span><span class="right pointer"
                                                       @click="operation">地址管理</span>
            </div>
            ...
            <li>
                <el-form ref="invoice" :model="invoice" label-width="100px">
                    <div class="goods clear">
                        <div class="show left">
                            <img :src="imgPath+goodsDetails.goods_cover_img" alt="">
                        </div>
                        <div class="detailed left">
                            <div class="goods_name" v-text="goodsDetails.goods_name">名字</div>
                            <div class="goods_type">
                                <span v-text="goodsDetails.goods_spec">黑色</span><span
                                v-text="">50g</span><span>|</span><span>数量：</span><span
                                v-text="goodsDetails.num"></span>
                            </div>
                            <div class="order_num" v-text="goodsDetails.order_num">
                            </div>
                            <div class="price money" v-text="goodsDetails.money">
                            </div>
                        </div>
                    </div>
                    <div class="couponList" v-if="!goodsDetails.coupon_id">
                        <div class="title">
                            优惠卷
                        </div>
                        <div>
                            <goodsCoupon2 v-model="couponInfo" :type="1" :content-id="goodsCouponId"
                                          :price="goodsDetails.money"></goodsCoupon2>
                        </div>
                    </div>
                    <div class="integral_type" v-show="goodsDetails.integral===0">
                        <div class="integral_text"><span v-text="goodsIntegral.integral"></span>积分抵扣<span
                            v-text="goodsIntegral.integral_money" class="money"></span></div>
                        ...
                        <div v-show="havInvoice">
                            ...
                            <div class="invoice-type">
                                <a :class="{active:invoice.type===1}"
                                   @click="invoice.type=1;$refs.invoice.resetFields()">个人</a>
                                <a :class="{active:invoice.type===2}"
                                   @click="invoice.type=2;$refs.invoice.resetFields()">单位</a>
                            </div>
...
<script>
    ...
    export default {
        name: "multiple-buy",
        components: {
            goodsCoupon2, Navigation, SelfModal, Alert
        },
        data() {
            return {
                showAlert: false,
        ...
        mounted() {
            ...
            this.goodsDetails = JSON.parse(decodeURIComponent(this.$route.query.goods));
            console.log("传过来的商品详情", this.goodsDetails)
            this.address_id = this.goodsDetails.addr.id
            api.goodsDetails({
                goods_id: this.goodsDetails.goods_id
            }).then((res) => {
                if (res.status === 1) {
                    this.goodsIntegral = res.data
                }
        ...
        computed: {
            finalPrice() {
                let result = this.goodsDetails.money
                if (this.couponInfo) {
                ...
                if (this.isIntegral) {
                    result = (result * 100 - this.goodsIntegral.integral_money * 100) / 100
                }
                if (result) {
        ...
        methods: {
            ...
            howIntegral() {
                if (this.goodsDetails.integral > this.integral) {
                ...
                if (this.userInfo.balance * 1 > this.goodsDetails.money * 1) {
                    api.buyGoodsOne(params).then(resp => {
                        if (resp.status === 1) {
                            this.$router.push({
                                name: 'goodsPayType',
                                params: {orderNum: this.goodsDetails.order_num, goodsId: this.goodsDetails.goods_id}
                            })
                        } else {
                            this.$message({
                                message: resp.msg,
