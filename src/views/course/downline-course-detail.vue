    ...
    <div class="downline-course-detail">
        ...
        <div class="course-info mt-20 clear">
            ...
            <div class="info left">
                ...
                <div class="info-time">时间：
                    <span v-text="$filter.timeFilter(courseDetail.line_start_time,'M月DD日 HH:mm')"></span>-<span v-text="$filter.timeFilter(courseDetail.line_end_time,'HH:mm')"></span>
                </div>
                ...
                <div class="clear buy">
                    <a class="left" @click="signUp">立即报名</a>
...
<script>
    import {Navigation, Tab, TabItem, CourseItem, StoreBlock} from '../../components/public'
    ...
    export default {
        ...
        methods: {
            ...
            // 报名
            signUp() {
                const userInfo = this.$helper.utils.storage.get('userInfo')
                if (!userInfo) {
                    this.$router.push({name: 'login'})
                    return
                }
                this.$router.push({name: 'downCourseSignUp', params: {courseId: this.courseId}})
            },
            buy() {
                ...
                query.free = this.courseDetail.price === '0.00'
                this.$router.push({name: 'downCourseSignUp', params: {courseId: this.courseId}, query})
            },
            download(pdfUrl) {
                window.open(this.imgPath + pdfUrl, '_blank')
            },
            search() {
                this.$router.push({name: 'courseSearch', params: {type: 3, searchStr: this.keywords}})
            },
            setCollect() {
