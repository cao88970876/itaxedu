<template>
    <div class="paySee_page projectDetails_page">
        <div class="answerDetails_breadcrumb">
            <el-breadcrumb separator-class="el-icon-arrow-right">
                <el-breadcrumb-item :to="{ path: '/recruit/recruitList' }">发布任务</el-breadcrumb-item>
                <el-breadcrumb-item>任务支付</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="pay_price">
    ...
    export default {
        ...
        methods: {
            ...
            weChatPay(params) {
                params.pay_type = '2'
                this.params=params
                resumeApi.create_task(params).then(resp => {
                    console.log(resp)
                    if (resp.status === 1) {
                ...
                resumeApi.accountInfo({
                        ...
                        if (resp.status === 1) {
                            if(resp.data.balance<this.money){
                                ...
                                resumeApi.create_task(params).then(resp => {
                                    this.orderInfo.order_num=resp.data.id
                                    this.$router.push({name:'myRecruitmentTaskPay', params: {orderNum: this.orderInfo.order_num,params:encodeURIComponent(JSON.stringify(params))}})
                                })
                            }
                        }
                    })
            ...
            finishPay() {
                this.$router.push({
                    ...
                    params: {orderNum: this.orderInfo.order_num, params:encodeURIComponent(JSON.stringify(this.params))}
                })
            },
