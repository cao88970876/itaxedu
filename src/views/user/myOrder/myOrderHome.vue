                ...
                <li v-for="item in orderList" @click="goDetail(item)">
                    <div class="goods">
                        ...
                        <div class="orderPay_btn" v-show="type===1">
                            ...
                            <div @click.stop="goBuy(item)">付款</div>
                        </div>
                        <div class="orderComplete_btn" v-show="type===2">
                            <div @click.stop="$router.push({name:'afterSale',query: {id:item.id}})" v-if="item.is_backable===1">售后申请</div>
                        </div>
                        <div class="orderSend_btn" v-show="type===3">
                            <div @click.stop="$router.push({name:'afterSale',query: {id:item.id}})"
                                 v-if="item.is_backable==1">售后申请
                            </div>
                            <div @click.stop="determine(item)">确认收货</div>
                        </div>
            ...
            <div class="check">
                <ul>
                    <li v-for="item in cancel">
                        <div class="btn">
                            <label>
                                <input type="radio" :value="item.id" v-model="isPublic"/>
                                <i class="icon radio-no-checked" :class="{'cancel-checked':isPublic===item.id}"></i>
    ...
    export default {
        ...
        methods: {
            ...
            goBuy(item) {
                ...
                this.$router.push({
                    name: 'goodsOrder',
                    query: {goods:encodeURIComponent(JSON.stringify(item))}
                })
            },
            //获取到订单
            cancelBtn(item) {
